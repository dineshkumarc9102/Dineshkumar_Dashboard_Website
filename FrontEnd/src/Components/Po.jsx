...
export default function PO() {
    ...
    if (iframeRef.current) {
      ...
      toast.custom(() => (
        <div className="flex items-center gap-2 bg-[#162b3d] text-white px-4 py-2 rounded-lg shadow">
          ...
          <span>Refreshing PO dashboard</span>
        </div>
      ));
    }
  };
  ...
  // ✅ OPEN IN NEW TAB
  const handleOpen = () => {
    window.open(import.meta.env.VITE_POWERBI_PO, "_blank");

    toast.custom(() => (
      <div className="flex items-center gap-2 bg-[#162b3d] text-white px-4 py-2 rounded-lg shadow">
        <ExternalLink size={16} className="text-blue-400" />
        <span>Opened PO dashboard</span>
      </div>
    ));
  };
      ...
      <div className="flex-1 p-2">
        <div className="w-full h-full border-2 border-gray-600 rounded-xl overflow-hidden">
          ...
          <iframe
            ref={iframeRef}
            src={
              import.meta.env.VITE_POWERBI_PO
                ? `${import.meta.env.VITE_POWERBI_PO}&filterPaneEnabled=false`
                : ""
            }
